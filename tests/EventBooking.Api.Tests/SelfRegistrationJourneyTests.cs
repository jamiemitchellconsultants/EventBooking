...
public sealed class SelfRegistrationJourneyTests(ApiFactory factory)
{
    ...
    public async Task SubmitViewConfirmDispatchesTheConfirmationEmail()
    {
        ...
        var token = receipt.GetProperty("confirmationToken").GetString()!;
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            var booking = Assert.Single(await db.Bookings.ToListAsync());
            ...
            db.Entry(delivery).Property("NotBefore").CurrentValue =
        ...
        var posted = Assert.Single(factory.EmailTransport.Sent);
    ...
    public async Task RetentionPurgesTerminalRequestsPastThirtyDaysWithTheirEmails()
    {
        ...
        using (var scope = factory.Services.CreateScope())
        {
            ...
            var email = await db.EmailLogs.SingleAsync(x =>
                x.TemplateName == EmailTemplate.SelfRegistrationConfirmation
                && x.BookingId != null && oldBookingIds.Contains(x.BookingId.Value));
        ...
        HttpClient anonymous, Guid groupId, Guid eventId, string name, string email)
    {
        ...
        Assert.Equal(HttpStatusCode.Created, submitted.StatusCode);
        return (await BodyAsync(submitted)).GetProperty("confirmationToken").GetString()!;
    }
