...
2. In **Administration**, find **Locations**, **Appointment types**, **Attendee groups**, **System settings**, and **Staff access**. In **Your work**, find **Event groups**, **Event operations** and **Audit search**. Open **Help**. **Expect:** the pages load and the Admin guide is available.
...
3. Open **Attendee groups**. Open one row with **Edit**. **Expect:** its required appointment types are selected and a **Description** field (up to 500 characters, shown publicly on self-registration forms) is present. Select **Cancel** without saving.
...
![Attendee group editor with Description](screenshots/adm02-attendee-group-edit.png)
*Attendee group editor with the new Description field.*
...
1. Open **System settings**. Record **Invitation expiry (days)**, **Automatic retries**, and **Options per invitation**, and **Self-registration confirmation window (hours)**. **Expect:** all four current values are visible; options per invitation is between 1 and 5 and the confirmation window is between 1 and 168 hours.
