...
| Negotiation and operations | Proposal rows omit listed type detail; proposal links are not state-specific; capacity rows have no adjust link | Add ProposalTypeResponse rows, accept/withdraw-acceptance/withdraw links by state, and an `adjust` link on each caller-editable capacity |
| Appointment workspace | Event rows use loose date/time fields; roster rows do not expose the appointment resource/action links | Return the complete EventTimeResponse plus workspace event links, and appointmentId plus status-action links on each roster row |
