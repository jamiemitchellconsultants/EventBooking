...
**Phases 0, 1 and 2 are complete and executable. Phases 3 (Tasks 12–20, with Task 20 split into 20a/20b) and 4 (Tasks 21–23, with Task 22 split into 22a/22b) are hand-authored and not yet executed. Tasks 24–33 are not yet written.** Tasks 1 to 11, including the lettered splits 3a–3d and 9a–9b, are documented, implemented and replayed from these documents into an independent checkout, ending at a green build and 1570 passing tests. Master Tasks 24–33 still have to be authored; read [the authoring handover](HANDOVER.md) before continuing that work, and note that it carries an unanswered checkpoint on the method for Phase 5.

Start at [phase-0-port-and-strip.md](phase-0-port-and-strip.md), which gives the branch, the task order and the phase pull request. [phase-1-domain.md](phase-1-domain.md) carries Phase 1 — variable-length windows, Admin-managed reference data, N-type negotiation, N-row capacity, and location-restricted invites with a closed attendee status table — and ends with its own pull-request gate. [phase-2-persistence.md](phase-2-persistence.md) carries Phase 2 — the attendee link the server can reproduce, the fresh schema with its constraints and database roles, the ordered row-lock helpers with their concurrency harness, and the relational-division eligibility query with the derived start instant it orders on — and ends with its own pull-request gate. [phase-3-application.md](phase-3-application.md) carries Phase 3, starting with [Task 12](phase-3a-reference-data-settings.md), the reference-data and settings handlers. [phase-4-api-and-mcp.md](phase-4-api-and-mcp.md) carries Phase 4 — the API conventions every endpoint obeys, the read models design 05 needs, the endpoint catalogue itself and MCP parity — starting with [Task 21](phase-4a-api-conventions.md), the API conventions, and ending with its own pull-request gate at [Task 23](phase-4d-mcp-parity.md).
