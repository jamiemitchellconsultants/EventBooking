...
**Phases 0, 1 and 2 are complete and executable. Phases 3 (Tasks 12–20, with Task 20 split into 20a/20b), 4 (Tasks 21–23, with Task 22 split into 22a/22b), 5 (Tasks 24–27), 6 (Tasks 28–31) and 7 (Tasks 32–33) are hand-authored and not yet executed.** Tasks 1 to 11, including the lettered splits 3a–3d and 9a–9b, are documented, implemented and replayed from these documents into an independent checkout, ending at a green build and 1570 passing tests. Read [the authoring handover](HANDOVER.md) before continuing; it records the binding settlements and the verification limits of every unexecuted phase.

Start at [phase-0-port-and-strip.md](phase-0-port-and-strip.md), which gives the branch, the task order and the phase pull request. [phase-1-domain.md](phase-1-domain.md) carries Phase 1 — variable-length windows, Admin-managed reference data, N-type negotiation, N-row capacity, and location-restricted invites with a closed attendee status table — and ends with its own pull-request gate. [phase-2-persistence.md](phase-2-persistence.md) carries Phase 2 — the attendee link the server can reproduce, the fresh schema with its constraints and database roles, the ordered row-lock helpers with their concurrency harness, and the relational-division eligibility query with the derived start instant it orders on — and ends with its own pull-request gate. [phase-3-application.md](phase-3-application.md) carries Phase 3, starting with [Task 12](phase-3a-reference-data-settings.md), the reference-data and settings handlers. [phase-4-api-and-mcp.md](phase-4-api-and-mcp.md) carries Phase 4 — API conventions, read models, the endpoint catalogue and MCP parity — and ends at [Task 23](phase-4d-mcp-parity.md). [phase-5-web.md](phase-5-web.md) carries Phase 5 — the neutral design system, Admin screens, Manager/operations screens, Coordinator and anonymous flows, role Help, and the standalone Web accessibility gate — starting with [Task 24](phase-5a-design-system.md) and ending with [Task 27](phase-5d-coordinator-attendee-help.md). [phase-6-seed-and-deployment.md](phase-6-seed-and-deployment.md) carries Phase 6 — the safe seed CLI, generalised demo graph, local and home-lab container deployments, five published images and the release migrations bundle — from [Task 28](phase-6a-seed-cli.md) through [Task 31](phase-6d-images-and-release.md). [phase-7-verification-and-documentation.md](phase-7-verification-and-documentation.md) carries the 500-confirmation load test and final application and operator documentation in [Task 32](phase-7a-load-test.md) and [Task 33](phase-7b-documentation.md).
