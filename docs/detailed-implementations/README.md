...
**Phases 0 and 1 are complete and executable. Phases 2–7 are not yet written.** Tasks 1 to 8, including the lettered splits 3a–3d, are documented, implemented and replayed from these documents into an independent checkout, ending at a green build and 1540 passing tests. Master Tasks 9–33 still have to be authored; read [the authoring handover](HANDOVER.md) before continuing that work.

Start at [phase-0-port-and-strip.md](phase-0-port-and-strip.md), which gives the branch, the task order and the phase pull request. [phase-1-domain.md](phase-1-domain.md) carries Phase 1 — variable-length windows, Admin-managed reference data, N-type negotiation, N-row capacity, and location-restricted invites with a closed attendee status table — and ends with its own pull-request gate.
