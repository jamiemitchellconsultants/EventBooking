...
- **`SelfRegistration`** — Confirmation is single-use, requires a valid emailed token before expiry, revalidates both public gates, group membership, event compatibility and capacity, then creates or reuses the `Attendee` and creates an `Invite` and `Booking` in one transaction.
